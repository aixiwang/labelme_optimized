...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def saveFile(self, _value=False):
        ...
        if self._config['flags'] or self.hasLabels():
            if self.labelFile:
                print('saveFile 1',self.labelFile)
                # DL20180323 - overwrite when in directory
                self._saveFile(self.labelFile.filename)
            elif self.output_file:
                print('saveFile 2',self.output_file)
                ...
                if self.output_dir:                
                    print('basename:',basename,self.output_dir,LabelFile.suffix)
                    self.default_labelfile_name = osp.join(self.output_dir,basename + LabelFile.suffix)
                
                else:    
                    print('basename:',basename,self.currentPath(),LabelFile.suffix)
                    self.default_labelfile_name = osp.join(self.currentPath(),basename + LabelFile.suffix)
                print('default_labelfile_name:',self.default_labelfile_name)
                self._saveFile(self.default_labelfile_name)
